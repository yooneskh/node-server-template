import { multiByteIndexOf, stringToBytes, readUInt64LE, tarHeaderChecksumMatches, uint8ArrayUtf8ByteString } from './util';
...
export function getFileType(input: Uint8Array | ArrayBuffer | Buffer) {
  ...
  const checkString = (header: string, options: { offset: number, mask?: number[] } = { offset: 0 }) => check(stringToBytes(header), options);
  ...
  if (check([0x1A, 0x45, 0xDF, 0xA3])) {
    ...
    if (idPos !== -1) {
      ...
      const findDocType = (type: any) => [...type].every((c, i) => sliced[docTypePos + i] === c.charCodeAt(0));
...
export const extensions = new Set(supported.extensions);

export const mimeTypes = new Set(supported.mimeTypes);
